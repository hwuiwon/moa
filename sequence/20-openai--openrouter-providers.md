# Step 20: OpenAI + OpenRouter Providers

## What this step is about
Adding `LLMProvider` implementations for OpenAI and OpenRouter.
...
## Goal
Users can switch between Anthropic, OpenAI, and OpenRouter models via config or `/model` command. All three support streaming, tool use, and accurate capability reporting.

## Tasks
1. **`moa-providers/src/openai.rs`**: `OpenAIProvider` using `async-openai` crate. Chat completions with streaming and tool_choice.
2. **`moa-providers/src/openrouter.rs`**: `OpenRouterProvider` — OpenAI-compatible API with OpenRouter-specific headers (HTTP-Referer, X-Title).
...
## Deliverables
`moa-providers/src/openai.rs`, `moa-providers/src/openrouter.rs`

## Acceptance criteria
1. OpenAI: Chat completion with streaming works
2. OpenAI: Tool use works (parallel tool calls)
3. OpenRouter: Same, with correct headers
