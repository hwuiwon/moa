...
fn contract_for(name: &str) -> Option<ToolContract> {
    let contract = match name {
        ...
        "analytics_query" => ToolContract::object("A bounded query result object containing typed columns, result rows, and execution metadata.", "Drill into relevant sessions with `sessions_list` or establish a baseline with an experiment."),
        ...
        "lineage_explain" => ToolContract::object("A lineage explanation object containing the matched subject and its tenant-scoped provenance records.", "Use the evidence to select an experiment or artifact change; do not infer missing raw content."),
        ...
        "artifact_import" => ToolContract::object("A newly created draft artifact revision with its stable identity and exact revision UID; it is not active. Hand-authored skill drafts cannot be published.", "For a non-skill artifact, inspect the draft and call `artifact_publish` when ready. Learned skills activate only through learning-candidate review."),
        "artifact_publish" => ToolContract::object("The published non-skill artifact identity, revision UID, and resulting lifecycle state. Skill revisions are rejected.", "Run the relevant experiment and compare it with the baseline."),
        ...
        "learning_candidate_accept_skill" => ToolContract::object("The recorded review result and resulting candidate or serving-skill state after the regression and activation gates run.", "Verify the resulting skill with `artifacts_list`, then run the relevant experiment."),
        "learning_candidate_accept_rollback" => ToolContract::object("The recorded rollback result: the regressed revision is archived and the serving pointer is tombstoned, or a typed conflict reports that the proposal is stale. No predecessor is restored.", "Verify the skill is unserved. Any replacement must pass a separate `learning_candidate_accept_skill` review."),
        "learning_candidate_dismiss" => ToolContract::object("The recorded dismissal of an informational candidate; nothing is published and one durable audit is written.", "Return to `learning_candidates_list` for remaining review work."),
        ...
        "agent_definition_deploy" => ToolContract::object("The new deployment record, installation UID, exact revision UID, and resulting active state.", "Confirm with `agent_deployments_list`, then run the relevant experiment or simulation."),
