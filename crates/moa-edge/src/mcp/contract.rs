...
fn contract_for(name: &str) -> Option<ToolContract> {
    let contract = match name {
        ...
        "learning_candidate_accept_rollback" => ToolContract::object("The recorded rollback result: the archived revision and the restored predecessor, or a typed conflict when a newer promotion has superseded the proposal.", "Verify the serving revision with `artifacts_list`, then confirm the regression that prompted the rollback has cleared."),
        ...
        "index_rebuild_start" => ToolContract::object("The accepted rebuild operation UID, its kind, and its initial planning state; nothing is rebuilt yet and the partition keeps serving its current vectors.", "Poll `index_rebuild_status` until the operation reaches `activated`, or stop it with `index_rebuild_cancel`."),
        "index_rebuild_status" => ToolContract::object("The rebuild's lifecycle, candidate and active generation UIDs, exact vector counts, deterministic cost estimate, provider request/throttle/retry counters, shadow-validation overlap, and last safe error. Cost is an estimate derived from input size, never a bill.", "While `activated`, choose `index_rebuild_rollback` to restore the previous generation or `index_rebuild_finalize` to close the rollback window."),
        "index_rebuild_cancel" => ToolContract::object("The recorded cancellation request and the operation's lifecycle; the build stops at its next committed checkpoint rather than immediately.", "Poll `index_rebuild_status` until the operation reports `cancelled`."),
        "index_rebuild_rollback" => ToolContract::object("The restored previous generation and the operation's resulting `rolled_back` lifecycle; production reads revert to the vectors that were authoritative before activation.", "Confirm with `index_rebuild_status`, then investigate the rebuild's recorded error before starting another."),
        "index_rebuild_finalize" => ToolContract::object("The operation's resulting `finalized` lifecycle after retired generation data is discarded; rollback is no longer possible.", "Confirm with `index_rebuild_status`; a further rebuild must be started with `index_rebuild_start`."),
