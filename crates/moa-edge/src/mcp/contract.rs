...
fn contract_for(name: &str) -> Option<ToolContract> {
    let contract = match name {
        ...
        "capabilities_list" => ToolContract::object("A capability catalog containing procedure node types, tools, constraints, and builder-facing metadata available to the tenant.", "Use a capability to author or inspect a skill artifact, then start it with `procedure_run_start`."),
        "procedure_runs_list" => ToolContract::object("A bounded page of procedure run summaries plus the keyset cursor for the next page.", "Call `procedure_run_status` for one run or pass the returned cursor back here."),
        "procedure_run_status" => ToolContract::object("The procedure run status with current lifecycle state, active node, and bounded node-run summaries.", "Poll again while active, or use `procedure_review_decide`, `procedure_signal`, or `procedure_run_cancel` when the status requires it."),
        "procedure_run_start" => ToolContract::object("The accepted procedure run ID, initial status, resolved procedure reference, and idempotency outcome.", "Poll with `procedure_run_status` using the returned run ID."),
        "procedure_run_cancel" => ToolContract::object("The cancellation acknowledgement and resulting procedure run status.", "Confirm the terminal state with `procedure_run_status`."),
        "procedure_review_decide" => ToolContract::object("The recorded review decision, affected node, and resumed procedure run status.", "Poll with `procedure_run_status` to observe the next node or terminal result."),
        "procedure_signal" => ToolContract::object("The delivered signal acknowledgement, matched node, and resumed procedure run status.", "Poll with `procedure_run_status` to observe progress."),
