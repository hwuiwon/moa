...
fn contract_for(name: &str) -> Option<ToolContract> {
    let contract = match name {
        ...
        "artifact_publish" => ToolContract::object("The published artifact identity, revision UID, and resulting lifecycle state.", "Run the relevant experiment and compare it with the baseline."),
        ...
        "learning_candidate_accept_skill" => ToolContract::object("The recorded review result and resulting candidate or artifact state after regression and publish gates run.", "Verify the resulting skill with `artifacts_list`, then run the relevant experiment."),
