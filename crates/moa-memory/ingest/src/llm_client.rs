...
impl LlmChatClient {
    /// Creates a Cohere chat client from an API-key environment variable.
...
mod tests {
    ...
    async fn llm_client_maps_timeouts_and_http_errors_to_typed_errors() {
        ...
        assert!(matches!(
            error,
            crate::IngestError::LlmChat(LlmChatError::Auth { .. })
        ));
        ...
        assert!(matches!(
            error,
            crate::IngestError::LlmChat(LlmChatError::Malformed { .. })
        ));
        ...
        assert!(matches!(
            error,
            crate::IngestError::LlmChat(LlmChatError::Timeout { .. })
        ));
    }
