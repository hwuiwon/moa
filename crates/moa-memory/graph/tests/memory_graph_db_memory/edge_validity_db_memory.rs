...
#[tokio::test]
async fn invalidated_edge_stops_feeding_live_expansion_while_endpoints_stay_active() {
    // Pins: edge validity is enforced independently of node validity — closing
    // just the relationship removes the traversal path from current-knowledge
    // walks while an as-of walk before the invalidation still replays it.
    ...
    let edge_valid_from = utc("2026-02-01T00:00:00Z");
    let seed_uid = graph
        .create_node(node_intent(
            &storage_partition_id,
            NodeLabel::Entity,
            "edge validity seed entity",
            edge_valid_from,
        ))
        .await
    ...
    let target_uid = graph
        .create_node(node_intent(
            &storage_partition_id,
            NodeLabel::Fact,
            "edge validity target fact",
            edge_valid_from,
        ))
        .await
        .expect("create target node");
    let edge = edge_intent(&storage_partition_id, seed_uid, target_uid, edge_valid_from);
    ...
    let before = graph
        ...
        .await
        .expect("expand before invalidation");
    assert!(
        before.iter().any(|hit| hit.uid == target_uid),
        "active edge must feed live expansion: {before:?}"
    );

    let pre_invalidation = Utc::now();
    graph
        .invalidate_edge(edge_uid, "edge_validity_test")
        .await
        .expect("invalidate edge");

    let after = graph
        .expand_seeds(&[seed_uid], 2, None, &GraphWalkScoring::default())
        .await
        .expect("expand after invalidation");
    assert!(
        after.iter().all(|hit| hit.uid != target_uid),
        "closed edge must not feed live expansion even though both nodes are active: {after:?}"
    );
    ...
    let historical = graph
        .expand_seeds(
            &[seed_uid],
            2,
            Some(pre_invalidation),
            &GraphWalkScoring::default(),
        )
        .await
        .expect("expand as-of before invalidation");
    assert!(
        historical.iter().any(|hit| hit.uid == target_uid),
        "as-of walk before the invalidation must replay the closed edge: {historical:?}"
    );
