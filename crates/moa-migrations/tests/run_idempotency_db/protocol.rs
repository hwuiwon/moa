...
async fn migration_protocol_pristine_apply_is_exact_and_idempotent_db() {
    // Pins: a pristine database applies the exact contiguous V1..V56 epoch,
    ...
    assert_eq!(
        expected_labels.len(),
        56,
        "the epoch must contain exactly 56 migrations"
    );
    assert_eq!(
    ...
    assert_eq!(
        history
            ...
            .collect::<Vec<_>>(),
        (1..=56).collect::<Vec<_>>(),
        "refinery history must be exactly contiguous from V1 through V56"
    );
    assert!(
        second.is_empty(),
...
async fn migration_protocol_exact_prefix_resumes_db() {
    ...
    // semantic migration and becomes a complete V1..V56 history.
    let admin_url = test_database_url();
    let db_name = unique_db_name();
    let admin = PgPoolOptions::new()
        .max_connections(1)
    ...
    assert!(
        partial_error.contains("incomplete: found 28 of 56 expected rows"),
        "complete-history validation must distinguish a valid prefix: {partial_error}"
    );
    assert_eq!(versions, (1..=56).collect::<Vec<_>>());
}
