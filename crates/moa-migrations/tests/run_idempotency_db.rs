...
type AuthzOutboxFact = (String, String, String, String, i32, i64, String, uuid::Uuid);
type TableContractFact = (String, String, bool, bool, bool, bool, bool, bool);
...
const KNOWLEDGE_CONNECTION_PARENT_MIGRATION: &str =
...
async fn migration_protocol_pristine_apply_is_exact_and_idempotent_db() {
    // Pins: a pristine database applies the exact contiguous V1..V52 epoch,
    ...
    assert_eq!(
        expected_labels.len(),
        52,
        "the epoch must contain exactly 52 migrations"
    );
    assert_eq!(
    ...
    assert_eq!(
        history
            ...
            .collect::<Vec<_>>(),
        (1..=52).collect::<Vec<_>>(),
        "refinery history must be exactly contiguous from V1 through V52"
    );
    assert!(
        second.is_empty(),
...
async fn migration_protocol_exact_prefix_resumes_db() {
    ...
    // semantic migration and becomes a complete V1..V52 history.
    let admin_url = test_database_url();
    let db_name = unique_db_name();
    let admin = PgPoolOptions::new()
        .max_connections(1)
    ...
    assert!(
        partial_error.contains("incomplete: found 28 of 54 expected rows"),
...
async fn bounded_tenant_purge_final_schema_executes_bounded_batches_db() {
    // Pins: a pristine final schema persists exactly 135 purge stages, installs
...
fn postgres_error_fact(error: sqlx::Error) -> (Option<String>, Option<String>) {
    (
        error
            .as_database_error()
...
#[test]
fn knowledge_connection_parent_migration_is_the_narrow_final_tip_offline() {
    // Pins: the unshipped history ends at V52, where the connector parent has
    // already absorbed lifecycle/credential state and obsolete projection
    // receipt columns are removed from the final schema.
    assert_eq!(
...
async fn tenant_connector_connections_backfill_rls_and_replay_schema_db() {
    ...
    let outcome = async {
        ...
        sqlx::query(
            "INSERT INTO moa.connector_connections \
                (connection_uid, tenant_id, display_name, built_in_key, built_in_version, \
                 lifecycle_status, health_status) \
             VALUES ($1, $2, 'neighbour', 'managed:test', 1, 'active', 'ready')",
        )
        ...
        let cross_error = sqlx::query(
            "INSERT INTO moa.connector_connections \
                (connection_uid, tenant_id, display_name, built_in_key, built_in_version) \
             VALUES ($1, $2, 'forbidden', 'managed:test', 1)",
        )
        .bind(uuid::Uuid::new_v4())
        .bind(neighbour_tenant)
        .execute(&mut *cross_tenant)
    ...
    assert!(purge_definition.contains("catalog_count <> 132"));
    assert!(purge_definition.contains("exactly 132 tables"));
...
async fn tenant_connector_use_grants_enforce_same_tenant_rls_and_restrict_deletion_db() {
    ...
    let outcome = async {
        ...
        for (parent_id, parent_tenant, display_name) in [
            ...
            sqlx::query(
                "INSERT INTO moa.connector_connections \
                    (connection_uid, tenant_id, display_name, built_in_key, built_in_version) \
                 VALUES ($1, $2, $3, 'managed:grant-test', 1)",
            )
            .bind(parent_id)
            .bind(parent_tenant)
            .bind(display_name)
    ...
    assert!(purge_definition.contains("catalog_count <> 133"));
    assert!(purge_definition.contains("exactly 133 tables"));
...
async fn knowledge_connection_parent_constraint_and_replay_ledgers_are_strict_db() {
    ...
    assert_eq!(purge_count, 135);
    assert!(purge_definition.contains("catalog_count <> 135"));
    assert!(purge_definition.contains("exactly 135 tables"));
