...
--
-- Nullable by design: the barrier check is an additional AND-constraint on
-- reads implemented as a RESTRICTIVE
...
CREATE FUNCTION moa.current_cleared_barriers() RETURNS TEXT[]
LANGUAGE SQL STABLE
AS $$
    SELECT COALESCE(
...
ALTER TABLE moa.knowledge_connections
    ADD COLUMN information_barrier TEXT;
...
CREATE FUNCTION moa.erase_memory_data_subject(
    p_tenant_id UUID,
