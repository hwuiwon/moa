...
--
-- The constraints are NOT VALID so an existing deployment can start the
-- bounded `backfill-memory-sealed-content` job after applying migrations. NOT
-- VALID constraints still protect every new or changed row. The backfill's
-- database-serialized finalizer validates them only after a non-skipping
-- residue check proves every historical row is converted.

ALTER TABLE moa.node_index
    DROP COLUMN IF EXISTS name_sealed,
    DROP COLUMN IF EXISTS properties_sealed;
...
ALTER TABLE moa.node_index
    DROP CONSTRAINT IF EXISTS node_index_data_subject_required,
    DROP CONSTRAINT IF EXISTS node_index_data_subject_scope,
    DROP CONSTRAINT IF EXISTS node_index_sealed_content_state,
    DROP CONSTRAINT IF EXISTS node_index_base_confidence_valid;

ALTER TABLE moa.node_index
    ADD CONSTRAINT node_index_data_subject_required
        CHECK (data_subject_id IS NOT NULL) NOT VALID,
...
ALTER TABLE moa.embeddings
    DROP CONSTRAINT IF EXISTS embeddings_unsealed_content_only;
ALTER TABLE moa.embeddings
    ADD CONSTRAINT embeddings_unsealed_content_only
        CHECK (pii_class NOT IN ('phi', 'restricted')) NOT VALID;
...
DROP POLICY IF EXISTS embedding_guard_read ON moa.node_index;
...
DROP TRIGGER IF EXISTS embeddings_reject_sealed_node ON moa.embeddings;
