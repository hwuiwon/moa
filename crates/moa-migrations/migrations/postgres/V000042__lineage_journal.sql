...
--
-- This migration is the only definition of the queue. Every lineage writer uses
-- the centrally migrated database, so there is no standalone bootstrap schema.

CREATE TABLE analytics.lineage_journal (
...
-- Every drain uses this index for eligibility and FIFO claim order. The
-- generated key cannot drift from the lease pair it represents.
CREATE INDEX lineage_journal_claim_idx
...
CREATE INDEX lineage_journal_partition_idx
    ON analytics.lineage_journal (storage_partition_id, user_id);

-- Serves the edge lineage API's default tenant-partition query and exact
-- timestamp/key ordering without scanning a partition through a user-led index.
CREATE INDEX turn_lineage_partition_ts_keyset_idx
