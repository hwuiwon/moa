...
CREATE OR REPLACE FUNCTION moa.simulator_certification_mandate_immutable_guard()
RETURNS trigger AS $$
BEGIN
    RAISE EXCEPTION
        'simulator certification mandates are immutable (mandate=%)',
        OLD.mandate_uid
        USING ERRCODE = 'P0001';
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS simulator_certification_mandate_no_update
    ON moa.simulator_certification_mandate;
CREATE TRIGGER simulator_certification_mandate_no_update
    BEFORE UPDATE ON moa.simulator_certification_mandate
    FOR EACH ROW EXECUTE FUNCTION moa.simulator_certification_mandate_immutable_guard();
...
CREATE OR REPLACE FUNCTION moa.simulator_certification_evidence_import_immutable_guard()
RETURNS trigger AS $$
BEGIN
    RAISE EXCEPTION
        'simulator certification evidence imports are immutable (mandate=%)',
        OLD.mandate_uid
        USING ERRCODE = 'P0001';
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS simulator_certification_evidence_import_no_update
    ON moa.simulator_certification_evidence_import;
CREATE TRIGGER simulator_certification_evidence_import_no_update
    ...
    FOR EACH ROW EXECUTE FUNCTION moa.simulator_certification_evidence_import_immutable_guard();
