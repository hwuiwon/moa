...
CREATE TABLE moa.erasure_jobs (
    ...
    -- Highest stage the job still needs to run. Derived-learning closure must be
    -- resolved before graph rows disappear, so it is the first durable stage.
    stage TEXT NOT NULL DEFAULT 'learning'
        CHECK (stage IN (
            'learning', 'artifacts', 'vault', 'graph', 'digest', 'lineage', 'done'
        )),
    candidate_count BIGINT NOT NULL DEFAULT 0,
    learning_erased BIGINT NOT NULL DEFAULT 0,
    artifact_erased BIGINT NOT NULL DEFAULT 0,
...
CREATE INDEX erasure_jobs_subject_idx
