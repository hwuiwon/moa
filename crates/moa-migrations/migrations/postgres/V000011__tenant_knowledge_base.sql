...
-- Webhook discovery starts before a tenant scope is known. Keep its two exact
-- provider-account shapes separate so both predicates have a matching leading
-- index and the deterministic LIMIT 2 ambiguity check never scans the table.
CREATE INDEX IF NOT EXISTS knowledge_connections_provider_config_account_idx
    ON moa.knowledge_connections (
        provider,
        provider_config_key,
        provider_connection_id,
...
CREATE INDEX IF NOT EXISTS knowledge_connections_provider_account_idx
    ON moa.knowledge_connections (
        provider,
        provider_connection_id,
        tenant_id,
...
CREATE TABLE IF NOT EXISTS moa.knowledge_sync_runs (
    ...
    provider_trigger_completed_at TIMESTAMPTZ,
...
CREATE TABLE IF NOT EXISTS moa.knowledge_objects (
    ...
    CONSTRAINT knowledge_objects_acl_state_valid
        CHECK (acl_state IN ('current', 'stale', 'incomplete')),
    CONSTRAINT knowledge_objects_current_acl_complete
        CHECK (
            acl_state <> 'current'
...
CREATE TABLE IF NOT EXISTS moa.knowledge_chunks (
    ...
    CHECK (ordinal >= 0),
    CHECK (token_count >= 0),
    CONSTRAINT knowledge_chunks_graph_node_is_occurrence
        CHECK (graph_node_uid = chunk_uid)
);

-- Content hashes are lookup/diff keys, not occurrence identities. Repeated
-- content in one document version remains representable at distinct ordinals.
