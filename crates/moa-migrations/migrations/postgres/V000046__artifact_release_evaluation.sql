...
platform_packs AS (
    SELECT
        ...
        specs.target_class,
        specs.visibility,
        1::INT AS cohort_epoch,
        CASE WHEN specs.visibility = 'hidden' THEN 2 END AS cohort_size,
        CASE WHEN specs.visibility = 'hidden' THEN template.hidden_rotates_at END AS rotates_at,
        CASE WHEN specs.visibility = 'hidden' THEN 3 END AS max_attempts_per_epoch,
        '00000000-0000-4000-8000-0000000d74f1'::UUID AS plan_revision_uid,
        CASE
            WHEN specs.visibility = 'hidden' THEN template.hidden_cases
            ELSE template.authoring_cases
        END AS cases,
        template.mandatory_assertions,
        template.scenario_source
    FROM platform_pack_specs specs
    CROSS JOIN platform_pack_template template
)
INSERT INTO moa.artifact_release_case_pack (
    pack_uid, storage_partition_id, user_id, name, revision, target_class,
    visibility, cohort_epoch, cohort_size, rotates_at, max_attempts_per_epoch,
