...
-> Result<()> {
    ...
    // Runtime metrics, on the SAME collector base URL, from the SAME process.
    // A trace-only assertion was the whole gap: the exporter could be exporting
    // no metrics at all, or exporting them under a resource identity nothing can
    // join to these spans, and every trace assertion above would still pass.
    let transitions = capture
        .wait_for_metric(SERVICE_TIMEOUT, |metric| {
            metric.name() == "moa_execution_run_state_transitions_total"
        })
        .await
        .context("wait for exported execution run-state transition counter")?;
    assert!(
        transitions
            .data_points()
            .iter()
            .any(|point| point.attribute("state") == Some("completed") && point.value() >= 1.0),
        "run-state counter must record the completed transition this run performed: {:?}",
        transitions.data_points()
    );

    // The queue-to-start histogram pins that the explicit bucket views survive
    // the metrics bridge. Without them the SDK's default layout still produces a
    // metric with a plausible value, and every latency panel built on it
    // quantizes to a floor while continuing to report a number.
    let queue_to_start = capture
        .wait_for_metric(SERVICE_TIMEOUT, |metric| {
            metric.name() == "moa_execution_run_queue_to_start_seconds"
        })
        .await
        .context("wait for exported execution queue-to-start histogram")?;
    let [queue_point] = queue_to_start.data_points() else {
        anyhow::bail!(
            "queue-to-start histogram should export one data point: {:?}",
            queue_to_start.data_points()
        );
    };
    assert_eq!(
        queue_point.explicit_bounds(),
        moa_observability::EXECUTION_DURATION_SECONDS_BUCKETS,
        "exported histogram lost its configured boundaries"
    );
    assert!(
        queue_point.count() >= 1,
        "queue-to-start histogram recorded no observation: {queue_point:?}"
    );

    // Traces and metrics must carry a byte-identical resource, or no query can
    // join them. Comparing the whole map, not three chosen keys, is what makes a
    // future divergence in any attribute fail here.
    for metric in [&transitions, &queue_to_start] {
        assert_eq!(
            metric.resource_attributes(),
            run_span.resource_attributes(),
            "metric `{}` and trace resources disagree: {:?} vs {:?}",
            metric.name(),
            metric.resource_attributes(),
            run_span.resource_attributes()
        );
        assert_eq!(
            metric.resource_attribute("service.name"),
            Some(capture.resource_name())
        );
        assert_eq!(
            metric.resource_attribute("deployment.environment"),
            Some("test")
        );
        assert_eq!(
            metric.resource_attribute("service.version"),
            Some(capture.resource_name())
        );
    }

    // Nothing may have been posted to a path this collector does not serve. That
    // is the observable form of "the endpoint contract is wrong": an exporter
    // handed a base URL it then uses verbatim posts to `/`, which looks exactly
    // like an exporter that never ran.
    assert!(
        capture.unexpected_requests().await.is_empty(),
        "orchestrator exported to unserved collector paths: {:?}",
