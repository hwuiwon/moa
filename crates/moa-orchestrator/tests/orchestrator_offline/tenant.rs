...
#[test]
fn consolidation_jitter_is_deterministic_per_tenant() {
    let tenant_id = TenantId::new();

    let first = deterministic_consolidation_jitter_secs(&tenant_id);
    let second = deterministic_consolidation_jitter_secs(&tenant_id);
