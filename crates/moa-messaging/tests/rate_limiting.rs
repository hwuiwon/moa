...
async fn rate_limit_retry_gives_up_after_max_attempts_and_returns_typed_error() {
    ...
    let limiter = MessagingRateLimiter::for_platform(Platform::Slack)
...
async fn burst_of_concurrent_sends_to_same_channel_serialize_below_per_channel_limit() {
    ...
    let limiter = Arc::new(MessagingRateLimiter::for_platform(Platform::Slack).with_max_retries(0));
...
async fn rate_limit_metrics_are_emitted_per_platform_per_outcome() {
    ...
    assert_eq!(
        metrics
            .counter("messaging_send_429_received_total", Platform::Slack, None)
            .await,
        1
    );
    assert_eq!(
...
async fn slack_send_surfaces_non_rate_limit_failures_as_typed_http_status() {
    ...
    let failure = response
        .failure_for_platform(Platform::Slack)
...
async fn retry_after_header_is_respected(platform: Platform, header_name: &str) {
    ...
    let limiter = MessagingRateLimiter::for_platform(platform)
        .with_per_channel_interval(Duration::ZERO)
        .with_delay_first_send(false);
    let server_for_task = server.clone();
