...
use crate::agent::{
    ActionPolicy, AgentDefinition, ConnectorBinding, GuardrailPolicy, GuardrailStagePolicy,
...
use crate::connector::{
    ...
    RuntimeOperationBindingV1, RuntimeOperationPolicyV1, is_runtime_action_id,
};
...
fn validate_action_policy(definition: &ActionPolicy, report: &mut ValidationReport) {
    ...
    validate_connector_bindings(&definition.connector_bindings, report);
}

fn validate_connector_bindings(bindings: &[ConnectorBinding], report: &mut ValidationReport) {
...
fn validate_connector(definition: &ConnectorDefinition, report: &mut ValidationReport) {
    match definition {
        ConnectorDefinition::Legacy(definition) => validate_legacy_connector(definition, report),
        ConnectorDefinition::RuntimeV1(definition) => {
            validate_runtime_connector_v1(definition, report);
        }
    }
}

fn validate_legacy_connector(
    definition: &crate::connector::LegacyConnectorDefinition,
    report: &mut ValidationReport,
) {
    validate_action_id_uniqueness(
        definition.actions.iter().map(|action| action.id.as_str()),
    ...
    for (index, action) in definition.actions.iter().enumerate() {
        ...
        if option_is_trim_empty(action.tool_name.as_deref()) {
            report.push_error(
                format!("{root}.tool_name"),
                "connector action tool_name must not be empty",
            );
        }
        validate_json_schema(
            &format!("{root}.input_schema"),
...
fn validate_runtime_connector_v1(
    ...
    require_non_empty(
        "definition.spec.display_name",
        &definition.display_name,
        "runtime connector display_name",
        report,
    );
    if let RuntimeConnectorKindV1::BuiltInManaged { provider } = &definition.runtime
        && !is_operation_key(provider)
    {
        report.push_error(
            "definition.spec.runtime.provider",
            "managed provider must be a stable code-owned operation key",
        );
    }
...
fn validate_runtime_connector_action(
    ...
    runtime: &RuntimeConnectorKindV1,
    auth: &[RuntimeConnectorAuthRequirementV1],
    declared_slots: &HashSet<&str>,
    has_no_auth: bool,
    report: &mut ValidationReport,
) {
    let binding_matches_runtime = matches!(
    ...
    if !binding_matches_runtime {
        report.push_error(
            format!("{root}.binding"),
            "operation binding type must match the connector runtime type",
        );
    }

    match &action.binding {
        RuntimeOperationBindingV1::Http { contract } => {
            validate_http_operation_contract(
                &format!("{root}.binding.contract"),
                contract,
                auth,
                declared_slots,
                has_no_auth,
        ...
        RuntimeOperationBindingV1::BuiltInManaged {
            ...
            if !is_operation_key(operation) {
                report.push_error(
                    format!("{root}.binding.operation"),
                    "managed operation must be a stable code-owned operation key",
                );
            }
            validate_runtime_operation_policy(
                &format!("{root}.binding.contract"),
                contract,
                report,
            );
...
fn validate_http_path_parts(
    root: &str,
    path: &str,
    path_inputs: &[crate::connector::HttpPathInputV1],
    report: &mut ValidationReport,
) {
...
fn validate_runtime_operation_policy(
    ...
    if policy.action_class != ActionClass::ExternalWrite {
        report.push_error(
            format!("{root}.action_class"),
            "tenant-authored connector actions must use external_write",
        );
    }
    if policy.risk_level != RiskLevel::High {
        report.push_error(
            format!("{root}.risk_level"),
            "tenant-authored connector actions must use high risk",
        );
    }
    if policy.minimum_effect != ActionPolicyEffect::AdminReview {
        report.push_error(
            format!("{root}.minimum_effect"),
            "tenant-authored connector actions must require admin_review",
        );
    }
}
