...
use moa_gateway::GatewaySendResponse;
...
/// Returns a deterministic session id for gateway control-flow tests.
...
/// Asserts a normalizer returned a typed gateway/core error rather than panicking.
pub fn assert_typed_gateway_error(result: moa_core::Result<InboundMessage>) {
    assert!(
        matches!(
            result,
            Err(MoaError::SerdeJson(_)) | Err(MoaError::ValidationError(_))
        ),
        "expected typed gateway error, got {result:?}"
    );
}
...
pub async fn post_send(server: Arc<MockServer>) -> moa_core::Result<GatewaySendResponse> {
    ...
    let body = response
        ...
        .await
        .map_err(|error| MoaError::ProviderError(error.to_string()))?;
    let mut normalized = GatewaySendResponse::new(status, body);
