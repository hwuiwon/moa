...
impl ValidatedSkillPackage {
    fn from_package(package: SkillPackage) -> Result<Self> {
        ...
        let skill_definition = crate::artifact::skill_definition_from_parts(&document, &files)?;
        let manifest = SkillPackageManifest::from_document_files_and_definition(
            &document,
            &files,
            &skill_definition,
        );
...
pub struct SkillPackageManifest {
    ...
    /// Canonical artifact schema version for the skill package.
    #[serde(default = "default_artifact_schema_version")]
    pub artifact_schema_version: String,
    /// Canonical artifact kind for artifact-backed skills.
    ...
    /// Connector references declared by this skill.
    #[serde(default)]
...
impl SkillPackageManifest {
    fn from_document_files_and_definition(
        ...
        Self {
            ...
            inputs_schema: definition.inputs.clone(),
            outputs_schema: definition.outputs.clone(),
            actions: definition.actions.clone(),
            connectors: definition.connectors.clone(),
            ui: definition.ui.clone(),
            files: files
                .iter()
...
fn default_artifact_schema_version() -> String {
