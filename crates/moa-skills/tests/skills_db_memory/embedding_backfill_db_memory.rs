...
async fn skill_backfill_embeds_then_skips_unchanged_reactivation_db_memory() -> Result<()> {
    ...
    // later restale caused by an identity-preserving reactivation it retargets the
    // reusable row without making another provider call.
    ...
    assert_eq!(
        ...
        1,
        "initial backfill makes exactly one provider call",
    );
    assert!(
        registry
    ...
    assert_eq!(
        embedder.calls.load(Ordering::SeqCst),
        1,
        "unchanged reactivation makes no additional provider call",
    );
    assert!(
        registry
            .list_skills_missing_embedding("test-embedder", 1, 10)
