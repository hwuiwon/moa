...
#[tokio::test]
async fn render_dedups_active_duplicate_lessons() -> Result<()> {
    // Pins: two active lessons whose summaries normalize equal render as a
    // single addendum (newest wins), covering the between-curation-pass window.
    let _guard = GRAPH_TEST_LOCK.lock().await;
    let (store, database_url, schema_name) =
        moa_session::testing::create_isolated_test_store().await?;
    let workspace_name = Uuid::now_v7().to_string();
    ...
    let older = learn_lesson(
        skill_uid,
        "Older restatement body.".to_string(),
        "Check secret rotation before debugging OAuth code".to_string(),
        scope.clone(),
        Uuid::now_v7(),
        &lesson_ctx,
    ...
    .await?;
    let newer = learn_lesson(
        skill_uid,
        "Newer restatement body.".to_string(),
        "check  secret rotation before debugging oauth code.".to_string(),
        scope.clone(),
        Uuid::now_v7(),
        &lesson_ctx,
    ...
    assert_eq!(
        rendered.matches("(lesson: ").count(),
        1,
        "duplicate-summary lessons render as a single addendum"
    );
    // Newest wins: the rendered addendum links the newer lesson.
    assert!(rendered.contains(&newer.to_string()));
    assert!(!rendered.contains(&older.to_string()));

    for uid in [older, newer] {
        lesson_ctx
            .graph()
            .hard_purge(uid, "redacted:skill-render-dedup-test")
            .await
            .map_err(|error| MoaError::StorageError(error.to_string()))?;
    }
    drop(store);
