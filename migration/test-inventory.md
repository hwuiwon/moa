...
```bash
rg --files-with-matches 'for_test|LocalOrchestrator::new|moa_orchestrator_local|moa-orchestrator-local|LocalChatRuntime::for_test|LocalOrchestrator::for_test' crates/ --type rust | sort
...
- No `LocalChatRuntime::for_test` or `LocalOrchestrator::for_test` callers remain under `crates/`.
...
- Tests that need a real orchestrator should call `OrchestratorTestFixture::shared().await?.isolated().await` and use the returned `OrchestratorClient`.
