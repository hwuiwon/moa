...
Restate 1.7 exports traces over its supported `tracing-endpoint` OTLP/gRPC
setting; the local patch deliberately does not invent unsupported Restate
sampler variables. The pinned LGTM collector uses its image defaults and does
not tail Kubernetes stdout. Restate, Postgres, OpenFGA, Valkey, and other
infrastructure pod logs are therefore outside this endpoint-only local
contract. Local Postgres lineage is enabled with `MOA_LINEAGE_SINK=postgres`.
...
service and metric name, collector OTLP acceptance and export-failure counters,
