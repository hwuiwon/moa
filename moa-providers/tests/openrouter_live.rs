...
fn openrouter_live_model() -> String {
    std::env::var("OPENROUTER_MODEL").unwrap_or_else(|_| "openai/gpt-5.4".to_string())
}

#[tokio::test]
#[ignore = "requires OPENROUTER_API_KEY"]
async fn openrouter_live_completion_returns_expected_answer() {
    let provider = OpenRouterProvider::from_env(openrouter_live_model()).unwrap();
...
#[tokio::test]
#[ignore = "requires OPENROUTER_API_KEY"]
async fn openrouter_live_web_search_returns_current_information() {
    let provider = OpenRouterProvider::from_env(openrouter_live_model()).unwrap();
