...
6. **Workspace selection.** A user may be in multiple workspaces. The adapter must let the user select or default to one. Slack uses team + channel; future adapters must define their workspace routing explicitly.
