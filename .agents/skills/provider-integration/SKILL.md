...
description: >
  ...
  `HandProvider` (sandbox or runtime backend), MCP server, or `ChannelAdapter`
  (Slack or future enterprise chat platforms). It covers the required trait
  surface, model catalog and pricing wiring, credential vault entries, messaging
...
allowed-tools:
  ...
  - Bash(git:*)
metadata:
  moa-tags: "providers, llm, embeddings, hands, mcp, channel-adapters, integrations"
---

# Provider Integration

Use this skill when wiring a new external service or backend into MOA. Five trait families share this pattern: `LLMProvider`, `EmbeddingProvider`, `HandProvider`, `ChannelAdapter`, and MCP servers (which compose into `HandProvider` via the MCP router).
...
- adding a new channel adapter (Slack or future enterprise chat platforms)
- modifying any of the above to support new models, sandbox kinds, or channel features
...
| Chat platform (deliver messages, render approvals) | `ChannelAdapter` | `moa-messaging/src/<name>.rs` | [channel-adapter-checklist.md](references/channel-adapter-checklist.md) |
...
4. **Routing wiring** — the provider registry, brain, hand router, or channel adapter map needs to know the new provider exists. This is usually a one-line addition to a registration table.
...
LLM providers also need: a model catalog entry and a pricing entry. Embedding providers also need: a halfvec dimension declaration. Hand providers also need: a sandbox-kind tag for the tool router. Channel adapters also need renderer/action-review support for channel messages and approvals.
...
2. Read at least one existing implementation of the same type as a template:
   ...
   - Channel: `crates/moa-messaging/src/slack.rs`
...
6. Register the provider in the routing table: model catalog for LLMs, sandbox kind for hands, channel adapter map for messaging.
...
- A channel adapter must support the channel's approval/action-review rendering path; a chat channel that cannot render approvals is not a complete adapter.
- Scripted, fake, recorded, or deterministic test providers belong in tests, eval fixtures, or explicitly gated internal tooling. They must not be reachable through default production provider registration, model routing, hand routing, or channel adapter maps.

## Output Format

When reporting on a new provider integration, include:

- `Provider type`: LLM / Embedding / Hand / MCP / Channel
