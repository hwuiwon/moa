...
- **Verification tiers** (this is the important column — the audit ran out of Fable 5 budget partway through the verify pass):
  ...
  - ⚠️ **Unverified** — the skeptic pass for this finding never ran (rate limit). **These are raw finder claims — verify them yourself.**
...
|---|---|
| ✅ Confirmed | 23 |
| 🟡 Adjusted (confirmed with corrections) | 23 |
| ❌ Refuted (rejected, do not act) | 2 |
| ⚠️ Unverified (verifier did not run — check yourself) | 53 |
...
| 47 | ❌ | session / db / migrations / runtime-store | Neon checkpoint/branch subsystem (BranchManager + NeonBranchManager + NeonMaint cron + 4 admin handlers) that is never enabled anywhere |
| 48 | ❌ | lineage / ocsf / observability | Fjall fsync journal wraps the lossy fire-and-forget lineage path that is allowed to drop events anyway |
| 49 | ⚠️ | Orchestrator — services/rest | OrchestratorCtx dependency-group pyramid: 10-way trait-object fan-out of one store plus three parallel accessor surfaces, mostly dead |
| 50 | ⚠️ | Orchestrator — services/rest | runtime/endpoint.rs builds a function-pointer service registry to express a static bind list |
| 51 | ⚠️ | Orchestrator — services/rest | SessionStore service '_inner' layer: ~28 one-line pass-through methods between handlers and the store |
| 52 | ⚠️ | Orchestrator — services/rest | The 'redis' cargo feature is mandatory-in-practice: every build enables it and a non-redis binary refuses to start |
| 53 | ⚠️ | Orchestrator — services/rest | postmark/twilio feature flags are enabled by no build and guard ~1,450 lines of unreachable adapter code |
| 54 | ⚠️ | moa-brain (context pipeline) | Two overlapping compaction subsystems both own LLM checkpoint emission (stage-8 HistoryCompiler and stage-10 Compactor) |
| 55 | ⚠️ | moa-brain (context pipeline) | Per-turn lineage emits a constant 'retrieval_recall_proxy' score and records cost three different ways |
| 56 | ⚠️ | moa-brain (context pipeline) | Process-wide circuit-breaker registry (HashMap by namespace) with exactly one production namespace |
| 57 | ⚠️ | moa-memory (graph/ingest/lifecycle/pii/vector) | Turbopuffer second vector backend with outbox, promotion state machine, and dual-read for zero tenants |
| 58 | ⚠️ | moa-memory (graph/ingest/lifecycle/pii/vector) | Ingestion dependency wiring: process-global runtime with fingerprint compatibility checking plus three parallel dependency bundles and dead entry points |
| 59 | ⚠️ | moa-memory (graph/ingest/lifecycle/pii/vector) | embedding_model_version: a parallel identity column that is written everywhere and read nowhere |
| 60 | ⚠️ | moa-memory (graph/ingest/lifecycle/pii/vector) | MemoryScope::ancestors() is an identity function feeding a vestigial scope_ancestors chain |
| 61 | ⚠️ | moa-memory (graph/ingest/lifecycle/pii/vector) | PiiSpan.replacement Option exists only for backwards compatibility and never holds a non-default value |
| 62 | ⚠️ | moa-memory (graph/ingest/lifecycle/pii/vector) | memory.auto_bootstrap config knob has no production reader |
| 63 | ⚠️ | moa-core (traits/types/config/wire) | Hand-rolled ~2,300-line env-to-config mapping layer (MoaEnvOverlay + 22 apply_* mirror functions) |
| 64 | ⚠️ | moa-core (traits/types/config/wire) | 119 of 245 env config knobs are never set or referenced anywhere in the repo |
| 65 | ⚠️ | moa-core (traits/types/config/wire) | Session persistence split into 7 single-impl 'focused contract' traits plus a blanket SessionRepository aggregate |
| 66 | ⚠️ | moa-core (traits/types/config/wire) | Four near-identical turn-message DTOs in wire/turn.rs, two of them field-for-field identical |
| 67 | ⚠️ | moa-core (traits/types/config/wire) | EmbeddingProvider carries synonym method pairs (dimensions/dimension, model_id/model_name) |
| 68 | ⚠️ | lineage / ocsf / observability | LineageHandle JSON bridge forces serialize -> deserialize -> clone -> deserialize on every lineage event |
| 69 | ⚠️ | auth / agents / contacts / scoring / experiments | Single-variant ActionRuleScope enum threaded through scoring, experiments, and agents |
| 70 | ⚠️ | auth / agents / contacts / scoring / experiments | fga-bootstrap ships a second hand-rolled OpenFGA HTTP client that its own doc comment calls a stopgap |
| 71 | ⚠️ | auth / agents / contacts / scoring / experiments | Six experiment score row types are field-identical mirrors of moa-scoring types, with six hand-written mapping functions |
| 72 | ⚠️ | auth / agents / contacts / scoring / experiments | Agent policy mode enums duplicated byte-for-byte between moa-artifacts and moa-core, bridged by match-mappings in the resolver |
| 73 | ⚠️ | auth / agents / contacts / scoring / experiments | API-key validation uses two global caches plus per-entry mutexed revocation-recheck timestamps where one short-TTL cache gives the same guarantees |
| 74 | ⚠️ | auth / agents / contacts / scoring / experiments | AsyncAuthzProvider::poll_decision is a trait method with zero production callers; approvals resolve exclusively via awakeables |
| 75 | ⚠️ | eval crates | Entire Reporter subsystem (trait, TerminalReporter, ReporterOptions, build_reporters) has no production consumer |
| 76 | ⚠️ | eval crates | PairwiseLlmJudge and the AnswerJudge trait are fully unwired — the production runner only ever calls DeterministicJudge::judge_sync directly |
| 77 | ⚠️ | eval crates | MemoryOverride config knobs are pure speculative surface: two knobs hard-error 'not implemented' and the third is a no-op with a dead helper |
| 78 | ⚠️ | eval crates | Dead API surface in moa-eval-core and EvalEngine: unused discovery helpers, never-constructed error variants, an uncalled engine method, and an errors-only enum variant |
| 79 | ⚠️ | eval crates | Pentest support is library code for exactly one test binary, with drifted env knobs that CI sets under names the code no longer reads |
| 80 | ⚠️ | loadtest / test-support / xtask / scripts | Duplicated Restate ingress HTTP client and session-bootstrap sequence in moa-loadtest vs moa-test-support |
| 81 | ⚠️ | loadtest / test-support / xtask / scripts | check-architecture-boundaries: 99-entry counted-allowance ledger plus numeric LOC/symbol/package-count ratchets |
| 82 | ⚠️ | loadtest / test-support / xtask / scripts | Generic SessionStore 'contract tests' in moa-test-support with exactly one implementation and one consumer |
| 83 | ⚠️ | loadtest / test-support / xtask / scripts | perf-gate scenario layer duplicates report/gate plumbing inside moa-loadtest, including a dead public export |
| 84 | ⚠️ | loadtest / test-support / xtask / scripts | xtask migrate-test-db is a no-op that four CI steps build a release binary to run |
| 85 | ⚠️ | loadtest / test-support / xtask / scripts | inspectable_files: dead configurable workspace-root branch only ever called with None |
| 86 | ⚠️ | cross-cutting: single-impl abstractions | Session-store 'focused contract' trait family: 8 single-impl traits + supertrait all backed by one PostgresSessionStore |
| 87 | ⚠️ | cross-cutting: single-impl abstractions | IngestionObserver trait + pipeline type parameter with one zero-sized impl used everywhere, including tests |
| 88 | ⚠️ | cross-cutting: single-impl abstractions | Test doubles living in production knowledge-service module, one of them dead |
| 89 | ⚠️ | cross-cutting: single-impl abstractions | BranchManager trait with one impl, no dyn usage, and consumers that already name the concrete type |
| 90 | ⚠️ | cross-cutting: single-impl abstractions | DeliverySink trait is pure ceremony: single impl, never used as dyn or bound, consumer names the concrete type |
| 91 | ⚠️ | cross-cutting: config & feature-flag sprawl | experiments and internal-eval-runner features are enabled by no build, CI job, or script — ~10k lines of never-compiled subsystem |
| 92 | ⚠️ | cross-cutting: config & feature-flag sprawl | 150 of ~245 MOA_* env knobs are write-only: supported by MoaEnvOverlay but never set anywhere in the repo |
| 93 | ⚠️ | cross-cutting: config & feature-flag sprawl | slack/postmark/twilio feature triple-gating means every built binary ships without messaging, while the env surface pretends otherwise |
| 94 | ⚠️ | cross-cutting: config & feature-flag sprawl | Daytona/E2B hand-provider features are enabled by nothing, making 1,700 lines of adapters and their config knobs unreachable in every artifact |
| 95 | ⚠️ | cross-cutting: config & feature-flag sprawl | Neon branching stack (~1,300 lines + 9 env knobs) is configured off in every environment |
| 96 | ⚠️ | cross-cutting: config & feature-flag sprawl | Vestigial auth0 feature layer: an inner feature flag that gates nothing, on a provider chain no artifact ever ships |
| 97 | ⚠️ | cross-cutting: cross-crate duplication | Retry-After parsing, retryable-status classification, jitter, and response-body helpers are implemented independently in four crates |
| 98 | ⚠️ | cross-cutting: cross-crate duplication | Ephemeral test-Postgres bootstrap (URL fallback + identifier quoting + search-path pool + schema migrations) copy-pasted across ~14 test-support files |
| 99 | ⚠️ | cross-cutting: cross-crate duplication | ~35 hand-rolled mock LLMProvider implementations with a 25-line ModelCapabilities/pricing blob duplicated 29 times, alongside an existing general-purpose ScriptedProvider |
| 100 | ⚠️ | cross-cutting: cross-crate duplication | SQL identifier quoting defined 19 times and the search-path pool builder duplicated across three production binaries despite moa-db existing as the shared storage-helpers crate |
| 101 | ⚠️ | cross-cutting: cross-crate duplication | memory_llm::LlmChatClient re-implements retry, jitter, error classification, and provider failover inside moa-providers, parallel to the crate's own RetryPolicy/RateGuard/FailoverLLMProvider stack |
...
_The reviewer agreed there is real over-engineering but corrected the fix or its side effects — see the revised notes._
...
## ⚠️ Unverified findings (verifier did not run — verify yourself)

_These are raw finder claims. The adversarial skeptic pass hit the Fable 5 rate limit before reaching them, so no second pair of eyes checked hidden consumers or load-bearing constraints. Several are cross-cutting and overlap each other (e.g. multiple lenses independently flagged the env-config layer and the Neon/Daytona/E2B feature stacks) — treat overlaps as corroboration. **Confirm each against the real code before acting.**_
...
**Verification status: ⚠️ UNVERIFIED** — the adversarial verifier for this finding did not run (the audit hit the Fable 5 rate limit before this skeptic pass executed). Treat this as a raw finder claim: check the cited files and hidden-consumer / load-bearing-constraint questions yourself before acting.
...
_Listed for completeness so you know these were evaluated and the complexity was judged load-bearing._
