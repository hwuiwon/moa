...
Workspace tool actions use action policy instead. `AdminReview` creates a row
in `workspace_action_reviews`, returns a pending-review tool result to the
model, and lets the turn continue. Workspace admins list and decide those rows
through:

```sh
curl -H "Authorization: Bearer <key>" \
  http://localhost:10080/v1/workspaces/<workspace_id>/action-reviews
curl -X POST -H "Authorization: Bearer <key>" \
  -H "Content-Type: application/json" \
  http://localhost:10080/v1/workspaces/<workspace_id>/action-reviews/<review_id>/decision \
...
- Resolved builtin async-authz rows remain in Postgres for audit and later OCSF
  emission. Workspace action-review audit state is the review row plus session
